...
export const canonical=s=>aliases[s]??s;
...
export function makeGame(player,enemies,options={}){
 ...
 g.status=(a,name)=>{const k=canonical(name);const dots=a.dots.filter(e=>key(e)===k);if(dots.length)return dots.length;return Math.max(0,...[...a.buffs,...a.debuffs].filter(e=>key(e)===k||e.status===name).map(e=>e.value??1));};
 ...
 g.stat=(a,k)=>{
  ...
  if(k==='speed'){
   ...
   const flat=up.filter(e=>e.speedFlat).map(e=>e.value);v+=Math.max(0,...flat);
   v*=1+Math.max(0,...up.filter(e=>!e.speedFlat).map(ratio));v*=1-Math.max(0,...down.map(ratio));
  }
 ...
 g.targets=(source,effect,chosen)=>{
  ...
  if((effect.kind??effect.type)==='damage'&&!effect.ignoreTaunt){target=foes.find(a=>g.hasStatus(a,'taunt'))??target;}
 ...
 g.rawHit=(source,target,amount,{dot=false,secondary=false,hit=true,crits=0,...rest}={})=>{
  ...
  g.ensureActor(source);g.ensureActor(target);const beforeHP=target.hp,beforeShield=target.shield,shieldDamage=Math.min(target.shield,amount*(1-clamp(rest.shieldBypass??0)));if(!dot&&hit)target.debuffs=target.debuffs.filter(e=>key(e)!=='sleep');target.shield-=shieldDamage;
  let hpDamage=Math.min(target.hp,Math.max(0,amount-shieldDamage));
  ...
  if(!dot){if(target.side==='enemy')g.notifyEnemyDamaged?.(g,target,source,out);g.afterIncomingDamage?.(out);}
 ...
 g.damage=(source,target,e,opts={})=>{
  ...
  if(hit===undefined){const ev=clamp(.05+.5*(g.stat(target,'speed')-g.stat(source,'speed'))/Math.max(1,g.stat(target,'speed')+g.stat(source,'speed')),0,.25);hit=g.hitRng()>=Math.max(0,ev-(opts.hitBonus??0));cache?.set(hitKey,hit);}
  ...
  return g.rawHit(source,target,Math.max(1,amount),{hit:true,crits,maxHit:Math.min(maxHit,target.hp+target.shield),secondary,dot:Boolean(e.dotConversion||opts.dotConversion),shieldBypass:e.shieldBypass??0,skillId:g.action?.skill?.id});
 };
 ...
 g.addStatus=(source,target,e,opts={})=>{
  ...
  const record={...e,canonicalStatus:k,sourceId:source.id,sourceSkill:g.action?.skill?.id??opts.skillId??'',addedRound:g.round,turns:e.turns??1,charges:e.charges??(String(e.status).includes('禁言：下次支援')?1:undefined),sourceActor:source};
 ...
 g.cleanse=(target,n=1)=>{
  ...
  for(const prop of ['debuffs','dots'])for(let i=target[prop].length-1;i>=0&&count<max;i--)if(target[prop][i].cleanseable!==false){target[prop].splice(i,1);count++;}
  if(target.side!=='enemy')player.thisTurn.cleanse+=count;return count;
 };
 g.dispel=(target,n=1)=>{
  let count=0;const max=n==='all'||n===Infinity?Infinity:Number(n);
  target.buffs.sort((a,b)=>(key(b)==='taunt')-(key(a)==='taunt'));
  for(let i=0;i<target.buffs.length&&count<max;)if(target.buffs[i].dispellable!==false){target.buffs.splice(i,1);count++;}else i++;
 ...
 g.endPhase=a=>{
  ...
  a.dots=a.dots.filter(d=>d.remaining>0);a.debuffs=a.debuffs.filter(e=>e.expireMode!=='end'||e.expireAtPhase>a.phaseCount);
