...
// Match the enemy planner's existing 3+ unit fallback, including four-unit encounters.
export const encounterDamageScale=(count,boss=false)=>boss||count<=1?1:count===2?.62:.48;
export function encounter(definitions,level,patch={}){
 ...
 return definitions.map((d,i)=>{const stats=monsterStats(d,level);if(n>1&&!boss)stats.hp=Math.round(pool*d.statMultipliers.hp/weight);stats.hp*=(patch.hpScale??1)*(1+(patch.hpLevelRamp?.[d.tier]??0)*clamp((level-1)/99));stats.attack*=(patch.attackScale??1)*(1+(patch.attackTierRamp?.[d.tier]??0)*clamp((level-1)/19))*(1+(patch.attackLevelRamp?.[d.tier]??0)*clamp((level-1)/99));stats.defense*=patch.defenseScale??1;stats.crit*=patch.critScale??1;const a=actor('enemy:'+i+':'+d.id,'enemy',level,stats,d);a.slot=i;a.damageScale=encounterDamageScale(n,boss);a.encounterScale=a.damageScale;a.skillScale=a.damageScale;a.flags.entryScale=a.damageScale;a.offenseGroupFactor=(patch.groupOffense?.[n]??a.damageScale)/a.damageScale;return a;});
}
