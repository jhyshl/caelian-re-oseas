  ...
  return [
    {
      ...
      content: [
        ...
        '字段固定为：sceneState、progress、completionGateSatisfied、questCompleted、matchedTransitionId、suggestedNodeId、confidence、evidence、summary、gatheringRequested、giftItems、requiredItemSubmission。',
        ...
        'gatheringRequested 只能是布尔值。仅当玩家本人在本轮明确开始或执行采集、搜寻、拾取当前地点区域特产的行动时返回 true；只是提及、看见或观察采集物，NPC 单独采集，回忆过去采集，或只计划以后采集时都必须返回 false。',
        'gatheringRequested=true 只表示应打开本地采集页面，不表示玩家已经获得任何物品。不得为采集、搜寻或拾取返回物品名称、物品数量，也不得把这些物品写入 giftItems。当前地区和实际可采物完全由本地数据库与玩家操作决定。',
        ...
        '严格按这个类型模板返回并替换内容：{"sceneState":"uncertain","progress":"stay","completionGateSatisfied":false,"questCompleted":false,"matchedTransitionId":null,"suggestedNodeId":null,"confidence":0.5,"evidence":["可核验证据"],"summary":"已发生事实摘要","gatheringRequested":false,"giftItems":[],"requiredItemSubmission":null}',
      ].join('\n'),
    },
    {
      role: 'user',
      content: [
        ...
        '采集页面只会读取上述当前地点所属地区的本地资源；判定器只判断玩家本轮是否明确执行采集、搜寻或拾取，不选择或发放物品。',
