...
export const IMPERIAL_MAINTENANCE_RULES = [
  ...
  '【知情核验补充】currentEvent 只包含玩家亲历、亲自选择或已被告知的事件，存在这类事件时 known=true；不得把玩家已接取任务、正在做的行动标成不知情。未知幕后事件不要放入 currentEvent；没有新的玩家事件就继承上一条已知事件，没有可靠记录则写“尚无新的可知事件”。秘密不因正文采用全知叙述就变成玩家已知，必须存在实际获取途径。evidence 引用原文中的连续短句，不加“玩家：”“主API：”前缀，不把概述当作直接引用。text 只写事实，不夹带“User知情/不知情”等标签，知情状态仅放 known；提及玩家时用给定的真实姓名。',
  '【联动维护】各势力与阵营的行动彼此影响；考虑本轮变化对其他阵营利益、资源、情报与下一步措施的影响，只有消息实际传达到的阵营才可据此反应。记录有因果的调整，不让行动互相割裂，也不让所有阵营同时获得秘密情报。',
  ...
  '【皇室三人】瓦勒里乌斯、塞西莉亚、卢修斯分别完整输出 plan 当前计划、action 当前行动、next 下一步措施、history 过往行踪。计划与措施写具体目标和手段，行动写时间、地点、接触对象及进展；每项标注 User 是否知情。行动发生后把过往行动归档，history 每条必须包含四个字段：at（字符串，剧情时间未知写“时间未明”）、text（行动内容字符串）、known（布尔值）、evidence（依据字符串）。归档也必须保留原依据，不得省略 evidence 或改成 null、对象、数组；确无依据时填空字符串且 known=false，不编造证据。最多 16 条，可合并早期重复日常，但保留关键转折、因果与知情差异，不凭空补日期。',
...
export function buildImperialJudgeMessages(input: ImperialPromptInput): Array<{role:'system'|'user';content:string}> {
  ...
  return [{role:'system',content:IMPERIAL_MAINTENANCE_RULES}, {role:'user',content:[
    ...
    '【history 数组元素格式｜仅说明结构，不是新增行踪】',
    '{"at":"时间未明","text":"实际发生的行动","known":false,"evidence":"对应的原文依据或有前因的幕后推演说明"}。没有行踪时用 []；有行踪时每条都必须包含 at、text、known、evidence。',
