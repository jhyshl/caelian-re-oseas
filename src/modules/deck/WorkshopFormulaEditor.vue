...
const allStatuses=computed(()=>[...new Map([...objects.value.statuses,...WORKSHOP_STATUS_LIBRARY,...catalog.value.statuses].map(s=>[s.id,s])).values()]);
...
<template>
  <div class="rule-formula">
    ...
    <template v-else>
      ...
      <template v-else-if="['status','status_data'].includes(op)"><WorkshopObjectSelect v-if="op==='status'" v-model="formula.key" kind="statuses" :extra="catalog.statuses.map(s=>({...s,group:'本组合状态'}))" /><input v-else v-model="formula.key" :list="listId" :placeholder="op==='status'?'选择状态':'选择状态数据'"><datalist :id="listId"><template v-if="op==='status'"><option v-for="s in allStatuses" :key="s.id" :value="s.id">{{ s.name }}</option></template><template v-else><option v-for="r in catalog.records.filter(r=>r.scope==='status_data')" :key="r.key" :value="r.key">{{ r.label }}</option></template></datalist></template>
      <select v-else-if="['cards','card_items'].includes(op)" v-model="formula.key"><option value="hand">手牌</option><option value="deck">抽牌堆</option><option value="discard">弃牌堆</option><option value="exhaust">移出牌</option><option v-if="op==='card_items'" value="all">所有战斗牌堆</option></select>
      <template v-if="op==='card_items'"><button v-if="formula.value===undefined" type="button" @click="formula.value={op:'eq',args:[{op:'item',key:'cardId'},{op:'card_definition',key:''}]}">筛选指定卡牌／条件</button><WorkshopFormulaEditor v-if="formula.value!==undefined" v-model="formula.value" label="保留卡牌的条件" /></template>
      <WorkshopObjectSelect v-else-if="op==='resource'" v-model="formula.key" kind="resources" />
      ...
      <template v-if="op==='statuses'">
        ...
        <div v-if="formula.types!==undefined" class="state-types"><label v-for="s in allStatuses.filter(s=>formula!.key==='all'||('kind' in s?s.kind===formula!.key:formula!.key!=='dot'))" :key="s.id"><input v-model="formula.types" type="checkbox" :value="s.id">{{ s.name }} · {{ s.id }}</label><p v-if="!formula.types.length">未选择任何种类，返回空列表。</p></div>
