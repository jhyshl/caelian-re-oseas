...
const stats=[['id','单位编号'],['hp','当前生命'],['hpMax','生命上限'],['lostHp','已损生命'],['attack','攻击力'],['defense','防御'],['speed','速度'],['crit','暴击率'],['critDamage','暴击伤害'],['ehr','效果命中'],['res','效果抵抗'],['shield','护盾'],['ap','AP']];
...
<template>
  <div class="rule-formula">
    ...
    <template v-else>
      ...
      <select v-if="['stat','status','status_data','resource'].includes(op)" v-model="formula.target"><option value="self">持有者</option><option value="target">当前目标</option><option value="selected_target">本次选中目标</option><option value="source">施加者</option><option value="summoner">召唤者</option><option value="event_source">事件发起者</option><option value="event_target">事件目标</option></select>
