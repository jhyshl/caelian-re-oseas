...
<template>
  <div class="rule-steps">
    <article v-for="(step,i) in model" :key="i">
      ...
      <WorkshopFormulaEditor v-if="!['if','foreach','delay','summon','apply_status','remove_status','stop','remove_unit'].includes(step.type)" v-model="step.value!" label="数值" />
      <WorkshopFormulaEditor v-if="step.type==='if'" v-model="step.condition!" label="条件" />
      <template v-if="step.type==='native_status'"><label>复用效果<select v-model="step.status" @change="step.value=WORKSHOP_STATUS_LIBRARY.find(s=>s.id===step.status)?.value??1"><option v-for="s in WORKSHOP_STATUS_LIBRARY" :key="s.id" :value="s.id">{{ s.name }} · {{ s.polarity }}</option></select></label><WorkshopFormulaEditor v-model="step.chance!" label="基础命中率％" /></template>
      ...
      <template v-if="['native_status','delay','summon'].includes(step.type)"><WorkshopFormulaEditor v-model="step.turns!" label="回合数" /></template>
