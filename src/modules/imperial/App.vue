...
import { publicImperialFact, type ImperialNotice, type ImperialState } from '@/imperial/model';
import { clampLauncherPosition } from '@/modules/shell/floating-position';

const props = defineProps<{ host: Window; view: {profileKey:string;visible:boolean;state:ImperialState|null;notices:ImperialNotice[]} }>();
...
<template>
  ...
  <section v-if="view.visible && expanded && view.state" class="imperial-panel" role="dialog" aria-label="动荡的皇权状态栏">
    ...
    <div class="imperial-scroll">
      <p class="imperial-caption">{{ view.state.updatedAt ? '局势已更新' : '等待下一轮剧情' }} · 面板中的幕后情报不代表 User 已知</p>
      <template v-if="tab === '局势'">
        ...
        <h3>当前事件（User视角）</h3><p>{{ publicImperialFact(view.state.currentEvent) }}</p>
        <h3>User阵营</h3><p class="imperial-camp">{{ view.state.playerCamp }}<small v-if="view.state.playerClaimingThrone">争位中</small></p>
        <p v-if="view.state.campEvidence" class="imperial-caption">{{ view.state.campEvidence }}</p>
        <h3>重大进展播报</h3><article v-for="(notice,index) in view.state.majorProgress ?? []" :key="index" class="imperial-card imperial-notice"><b>{{ notice.title }}</b><p>{{ notice.detail }}</p></article><p v-if="!view.state.majorProgress?.length">本轮暂无新增播报</p>
      </template>
      <template v-if="tab === '皇室'">
        <h3>皇室</h3>
        <details v-for="name in IMPERIAL_HEIRS" :key="name" class="imperial-card" open><summary>{{ name }}</summary><dl><dt>当前计划</dt><dd>{{ publicImperialFact(view.state.royals[name].plan) }}</dd><dt>当前行动</dt><dd>{{ publicImperialFact(view.state.royals[name].action) }}</dd><dt>下一步措施</dt><dd>{{ publicImperialFact(view.state.royals[name].next) }}</dd><dt>过往行踪</dt><dd><ul v-if="view.state.royals[name].history.length"><li v-for="(item,index) in view.state.royals[name].history" :key="index">{{ item.at }} · {{ publicImperialFact(item) }}</li></ul><span v-else>暂无记录</span></dd></dl></details>
        <details class="imperial-card"><summary>莱奥尼达斯</summary><dl><dt>当前状态</dt><dd>{{ publicImperialFact(view.state.emperor.status) }}</dd><dt>当前动向</dt><dd>{{ publicImperialFact(view.state.emperor.movement) }}</dd></dl></details>
      </template>
      <template v-if="tab === '势力'">
        <h3>各势力</h3>
        <details v-for="name in IMPERIAL_FACTIONS" :key="name" class="imperial-card" open><summary>{{ name }}</summary><dl><dt>当前动向</dt><dd>{{ publicImperialFact(view.state.factions[name].movement) }}</dd><dt>立场倾向</dt><dd>{{ publicImperialFact(view.state.factions[name].stance) }}</dd><dt>内部成员分歧</dt><dd>{{ publicImperialFact(view.state.factions[name].divisions) }}</dd></dl></details>
      </template>
      <template v-if="tab === '继承'">
        <h3>当前皇位继承可能性</h3><p class="imperial-caption">当前趋势，不代表必然结局 · 候选人合计 100%</p>
        <article v-for="candidate in view.state.successionLikelihood ?? []" :key="candidate.name" class="imperial-likelihood"><div><b>{{ candidate.name }}</b><strong>{{ candidate.value }}%</strong></div><progress :value="candidate.value" max="100" :aria-label="`${candidate.name}继承可能性`"></progress><p>{{ publicImperialFact(candidate.reason) }}</p></article>
        <p v-if="!view.state.successionLikelihood?.length">等待副 API 结合当前局势评估</p>
        <h3>皇位继承结果</h3><p>{{ view.state.winner ? `${view.state.winner}已正式继承皇位` : '尚未有人继承皇位' }}</p>
      </template>
    </div>
  </section>
  <div v-if="currentNotice" class="imperial-bulletin-backdrop">
    <section class="imperial-bulletin" role="alertdialog" aria-modal="true" aria-labelledby="imperial-bulletin-title">
      <small>局势快报 · {{ currentNotice.faction }}</small><h2 id="imperial-bulletin-title">{{ currentNotice.title }}</h2><p>{{ currentNotice.detail }}</p><button type="button" @click="dismissNotice">{{ view.notices.length > 1 ? '下一条' : '知晓了' }}</button>
    </section>
  </div>
</template>
