...
<template>
  <button v-if="view.visible" class="imperial-launcher" :class="{sleeping}" :style="{left:`${position.x}px`,top:`${position.y}px`}" type="button" aria-label="皇权局势，拖动可调整位置" :aria-expanded="expanded" @pointerdown="down" @pointermove="move" @pointerup="up" @pointercancel="up" @pointerenter="wake" @focus="wake" @click="toggle">
    <span aria-hidden="true">♛</span><small>皇权</small>
  </button>
  <section v-if="view.visible && expanded && view.state" class="imperial-panel" role="dialog" aria-label="动荡的皇权状态栏">
    ...
    <div class="imperial-scroll">
      <p class="imperial-caption">只展示你已知的局势 · {{ view.state.updatedAt ? '局势已更新' : '等待下一轮剧情' }}</p>
      <h3>当前势力支持</h3>
      ...
      <h3>当前事件 · 你的视角</h3><p>{{ publicImperialFact(view.state.currentEvent) }}</p>
      <h3>你的阵营</h3><p>{{ view.state.playerCamp }}</p>
      <h3>皇室</h3>
      <details v-for="name in IMPERIAL_HEIRS" :key="name" class="imperial-card"><summary>{{ name }}</summary><dl><dt>当前计划</dt><dd>{{ publicImperialFact(view.state.royals[name].plan) }}</dd><dt>当前行动</dt><dd>{{ publicImperialFact(view.state.royals[name].action) }}</dd><dt>下一步措施</dt><dd>{{ publicImperialFact(view.state.royals[name].next) }}</dd><dt>过往行踪</dt><dd><ul v-if="view.state.royals[name].history.length"><li v-for="(item,index) in view.state.royals[name].history" :key="index">{{ item.known ? `${item.at} · ${item.text}` : '未知（玩家不知情）' }}</li></ul><span v-else>暂无记录</span></dd></dl></details>
