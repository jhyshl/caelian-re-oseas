import { cardFormulaChoices } from '@/card-description';
...
describe('战斗显示规则',()=>{
  ...
  it('作者说明中的护盾公式随当前护盾更新，读取不改卡组',()=>{
    const card={id:'custom-shield-probe',name:'护盾测试',custom:true,type:'skill',description:'造成{{伤害1}}伤害；造成{{护盾伤害2}}伤害',effects:[
    ...
    card.descriptionBindings=cardFormulaChoices(card);
    ...
    expect(text).toBe('造成120伤害；造成200伤害');
    expect(battleCardText(card,3,battle,stats)).toContain('造成200伤害');
    battle.player.shield=0;
    expect(battleCardText(card,1,battle,stats)).toContain('造成0伤害');
