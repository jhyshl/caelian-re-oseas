...
function workshopBattleCandidate(id: string) {
...
function workshopBattleInput(
  professionId: string,
  opponentMode: 'random-single' | 'random-multi',
...
async function startFormalWorkshopBattle(
  profession: { id: string; main: 'knight' | 'mage' | 'artisan' | 'freelance' },
  label: string,
) {
  ...
  player.classMain = profession.main;
  player.subclass = profession.id;
  ...
  const active = (await database.battleSessions
    .where('profileId')
    .equals(profile.id)
    .first())!;
  return { battles, database, profile, active };
}
...
describe('本地战斗仓库', () => {
  it('同 ID 候选职业只在测试战斗使用新卡牌与天赋，结束后不污染正式缓存', async () => {
    const professionId = 'custom_class_scoped_runtime';
    const rawBase = workshopBattleCandidate(professionId);
    const rawPublished = {
      ...rawBase,
      classes: rawBase.classes.map((profession) => ({
        ...profession,
        name: '正式旧版职业',
        talent: {
          name: '正式旧天赋',
          description: '旧版天赋',
          effects: [{ type: 'battle_start_shield', value: 2 }],
        },
      })),
    };
    const published = saveWorkshopPack(rawPublished);
    const cardId = published.classes[0]!.cards[0]!.id;
    const sharedCards = await loadCardCatalog();
    const sharedPassives = await loadPassiveCatalog();

    const editedCandidate = structuredClone(published);
    editedCandidate.classes[0]!.name = '隔离候选新职业';
    editedCandidate.classes[0]!.cards[0]!.effects[0]!.value = 7;
    editedCandidate.classes[0]!.talent = {
      name: '候选新天赋',
      description: '只能在测试场生效',
      effects: [{ type: 'battle_start_shield', value: 17 }],
    };
    saveWorkshopTestPack(editedCandidate);

    const database = new CaelianDatabase(
      'alpha',
      `caelian-workshop-scoped-runtime-${crypto.randomUUID()}`,
    );
    databases.push(database);
    const game = new GameRepository(database, new EventBus());
    const profile = await game.ensureProfile('chat:workshop-scoped-runtime');
    await game.execute(profile.id, {
      id: 'workshop-scoped-player-create',
      type: 'player.create',
      payload: {
        name: '候选隔离测试员',
        classMain: 'knight',
    ...
    expect(session.state.enemies[0]!.hp).toBe(91);

    await game.execute(profile.id, {
      id: 'workshop-scoped-cancel',
      type: 'battle.cancel-workshop-test',
      payload: { battleId: session.id },
    });
    await game.execute(profile.id, {
      id: 'workshop-scoped-finish',
      type: 'battle.finish',
      payload: { battleId: session.id },
    });

    expect(sharedCards[cardId]?.effects[0]?.value).toBe(1);
    expect(sharedPassives[workshopPassiveId(professionId)]).toMatchObject({
      name: '正式旧天赋',
      effect: {
        effects: [{ type: 'battle_start_shield', value: 2 }],
      },
    });
    expect(published.classes[0]!.name).toBe('正式旧版职业');
  });

  it('仅接受候选卡池内合法的 15 张测试构筑，并按请求构筑开战', async () => {
    const professionId = 'custom_class_scoped_test_deck';
    const candidate = workshopBattleCandidate(professionId);
    const profession = candidate.classes[0]!;
    saveWorkshopTestPack(candidate);

    const invalidCommand = domainCommandSchema.safeParse({
      id: 'workshop-short-deck-schema',
      type: 'battle.start',
      payload: {
        workshopTest: {
          ...workshopBattleInput(
          ...
          deckIds: profession.starterDeck.slice(0, 14),
        },
      },
    });
    ...
    const database = new CaelianDatabase(
      'alpha',
      `caelian-workshop-test-deck-${crypto.randomUUID()}`,
    );
    databases.push(database);
    const game = new GameRepository(database, new EventBus());
    const createPlayer = async (suffix: string) => {
      const profile = await game.ensureProfile(`chat:workshop-deck-${suffix}`);
      await game.execute(profile.id, {
        id: `workshop-deck-player-${suffix}`,
        type: 'player.create',
        payload: {
          name: `测试构筑-${suffix}`,
          classMain: 'knight',
    ...
    const requestedDeck = profession.cardPool.slice(1, 16);
    const acceptedProfile = await createPlayer('accepted');
    const accepted = await game.execute(acceptedProfile.id, {
      id: 'workshop-requested-deck-start',
      type: 'battle.start',
      payload: {
        workshopTest: {
          ...workshopBattleInput(
          ...
          deckIds: requestedDeck,
        },
      },
    });
    expect(accepted.status).toBe('applied');
    const acceptedSession = (await database.battleSessions
      .where('profileId')
      .equals(acceptedProfile.id)
      .first())!;
    expect(
      [
        ...acceptedSession.state.player.hand,
        ...acceptedSession.state.player.drawPile,
        ...acceptedSession.state.player.discardPile,
      ]
        .map((entry) => entry.cardId)
        .sort(),
    ).toEqual([...requestedDeck].sort());

    const overThreeProfile = await createPlayer('over-three');
    const overThreeDeck = [...profession.starterDeck];
    overThreeDeck.splice(0, 3, ...Array(4).fill(profession.cards[7]!.id));
    expect(overThreeDeck).toHaveLength(16);
    overThreeDeck.pop();
    await expect(
      game.execute(overThreeProfile.id, {
        id: 'workshop-over-three-deck-start',
        type: 'battle.start',
        payload: {
          workshopTest: {
            ...workshopBattleInput(
            ...
            deckIds: overThreeDeck,
          },
        },
      }),
    ).rejects.toThrow('同名卡牌最多放入 3 张');

    const beyondPoolProfile = await createPlayer('beyond-pool');
    const beyondPoolDeck = [...profession.starterDeck];
    beyondPoolDeck[0] = profession.cards[7]!.id;
    beyondPoolDeck[1] = profession.cards[7]!.id;
    await expect(
      game.execute(beyondPoolProfile.id, {
        id: 'workshop-beyond-pool-deck-start',
        type: 'battle.start',
        payload: {
          workshopTest: {
            ...workshopBattleInput(
            ...
            deckIds: beyondPoolDeck,
          },
        },
      }),
    ).rejects.toThrow('不能超过候选职业卡池持有数');
  });

  it('从隔离候选职业启动低高难度单怪与混合怪群，且真实怪物不会套用木桩复活', async () => {
    const professionId = 'custom_class_random_candidate';
    const candidate = workshopBattleCandidate(professionId);
    saveWorkshopTestPack(candidate);
    const candidateCardId = candidate.classes[0]!.cards[0]!.id;
    const database = new CaelianDatabase(
      'alpha',
      `caelian-workshop-random-candidate-${crypto.randomUUID()}`,
    );
    databases.push(database);
    const game = new GameRepository(database, new EventBus());
    ...
    const startScenario = async (
      ...
      const profile = await game.ensureProfile(`chat:workshop-random-${suffix}`);
      await game.execute(profile.id, {
        id: `workshop-random-player-${suffix}`,
        type: 'player.create',
        payload: {
          name: `随机实战测试员-${suffix}`,
          classMain: 'knight',
      ...
      const started = await game.execute(profile.id, {
        id: `workshop-random-start-${suffix}`,
        type: 'battle.start',
        payload: {
          workshopTest: workshopBattleInput(
            professionId,
            mode,
            tier,
            seed,
      ...
      const session = (await database.battleSessions
        ...
        .equals(profile.id)
        .first())!;
      expect(session.state.workshopTest).toMatchObject({
        professionId,
        opponentMode: mode,
        respawns: 0,
      });
      expect(session.state.player.subclass).toBe(professionId);
      expect(
        session.state.enemies.every(
          (enemy) =>
            enemy.definitionId !== 'workshop_dummy' &&
            !enemy.tags.includes('dummy'),
        ),
      ).toBe(true);
      return { profile, session };
    };

    const low = await startScenario('low', 'random-single', 'low', 101);
    expect(low.session.state.enemies).toHaveLength(1);
    expect(['easy', 'normal']).toContain(
      catalog[low.session.state.enemies[0]!.definitionId]?.difficulty,
    );

    const high = await startScenario('high', 'random-single', 'high', 202);
    expect(high.session.state.enemies).toHaveLength(1);
    expect(['hard', 'nightmare']).toContain(
      catalog[high.session.state.enemies[0]!.definitionId]?.difficulty,
    );

    const mixed = await startScenario(
      'mixed',
      'random-multi',
      'mixed',
      303,
    ...
    expect(mixed.session.state.enemies).toHaveLength(4);
    const mixedDifficulties = mixed.session.state.enemies.map(
      (enemy) => catalog[enemy.definitionId]!.difficulty,
    );
    expect(
      mixedDifficulties.some((difficulty) =>
        ['easy', 'normal'].includes(String(difficulty)),
      ),
    ).toBe(true);
    expect(
      mixedDifficulties.some((difficulty) =>
        ['hard', 'nightmare'].includes(String(difficulty)),
      ),
    ).toBe(true);

    mixed.session.state.enemies[0]!.hp = 1;
    mixed.session.state.player.hand = [
      { instanceId: 'candidate-random-hit', cardId: candidateCardId },
    ];
    mixed.session.state.player.ap = mixed.session.state.player.apMax;
    await database.battleSessions.put(mixed.session);
    await game.execute(mixed.profile.id, {
      id: 'workshop-random-real-monster-hit',
      type: 'battle.play-card',
      payload: {
        battleId: mixed.session.id,
        handIndex: 0,
        targetIndex: 0,
      },
    });
    const afterHit = (await database.battleSessions.get(mixed.session.id))!;
    expect(afterHit.state.enemies[0]!.hp).toBe(0);
    expect(afterHit.state.workshopTest?.respawns).toBe(0);
    expect(afterHit.state.status).toBe('ongoing');
  });

  it('battle.cancel-workshop-test 拒绝关闭正式战斗', async () => {
    const database = new CaelianDatabase(
      'alpha',
      `caelian-cancel-formal-battle-${crypto.randomUUID()}`,
    );
    databases.push(database);
    const game = new GameRepository(database, new EventBus());
    const profile = await game.ensureProfile('chat:cancel-formal-battle');
    await game.execute(profile.id, {
      id: 'cancel-formal-player-create',
      type: 'player.create',
      payload: {
        name: '正式战斗测试员',
        classMain: 'knight',
    ...
    await game.execute(profile.id, {
      id: 'cancel-formal-start',
      type: 'battle.start',
      payload: { monsterId: 'mon_slime', source: '正式战斗测试' },
    });
    const session = (await database.battleSessions
      .where('profileId')
      .equals(profile.id)
    ...
    await expect(
      game.execute(profile.id, {
        id: 'cancel-formal-rejected',
        type: 'battle.cancel-workshop-test',
        payload: { battleId: session.id },
      }),
    ).rejects.toThrow('只能由自动评定关闭创意工坊隔离战斗');

    const unchanged = (await database.battleSessions.get(session.id))!;
    expect(unchanged.active).toBe(true);
    expect(unchanged.state.status).toBe('ongoing');
    expect(unchanged.state.workshopTest).toBeUndefined();
  });

  it('正式战斗中认证失效会阻止继续结算但仍允许撤退和关闭', async () => {
    const professionId = 'custom_class_invalid_certification_battle';
    const mechanismId = 'test.invalid-certification-battle';
    ...
    const raw = workshopBattleCandidate(professionId);
    const installed = saveWorkshopPack({
      ...raw,
    ...
    const profession = installed.classes[0]!;
    const combatHash = workshopCombatFingerprint(profession);
    saveWorkshopPack({
      ...installed,
      certifications: {
        [professionId]: {
    ...
    const database = new CaelianDatabase(
      'alpha',
      `caelian-invalid-certification-battle-${crypto.randomUUID()}`,
    );
    databases.push(database);
    const game = new GameRepository(database, new EventBus());
    const profile = await game.ensureProfile('chat:invalid-certification-battle');
    await game.execute(profile.id, {
      id: 'invalid-certification-player-create',
      type: 'player.create',
      payload: {
        name: '认证失效战斗测试员',
        classMain: 'knight',
    ...
    expect(readWorkshopTestCandidate(professionId)).toBeUndefined();
    await expect(
    ...
    ).rejects.toThrow('自动评定认证已失效');
    await expect(battles.endTurn(profile.id, active.id)).rejects.toThrow(
      '自动评定认证已失效',
    );

    await battles.surrender(profile.id, active.id);
    expect((await database.battleSessions.get(active.id))?.state.status).toBe(
      'surrendered',
    );
    await battles.finish(profile.id, active.id);
    expect((await database.battleSessions.get(active.id))?.active).toBe(false);

    await expect(
      battles.start(profile.id, {
        monsterId: 'mon_slime',
        source: '认证失效后的正式战斗测试',
      }),
    ).rejects.toThrow('自动评定认证已失效');
  ...
  it('删除认证职业依赖的整个机制后会阻止开战与进行中动作', async () => {
    const professionId = 'custom_class_deleted_mechanism_certification';
    const mechanismId = 'test.deleted-mechanism-certification';
    const mechanism = {
      format: 'caelian_workshop_mechanism',
      version: 1,
      id: mechanismId,
    ...
    const { battles, database, profile, active } =
      await startFormalWorkshopBattle(profession, 'deleted-mechanism-cert');
  ...
  it('删除认证职业引用的资源与状态后会阻止开战与进行中动作', async () => {
    const professionId = 'custom_class_deleted_resource_status_certification';
    const mechanismId = 'test.deleted-resource-status-certification';
    const mechanism = {
      format: 'caelian_workshop_mechanism',
      version: 1,
      id: mechanismId,
    ...
    const { battles, database, profile, active } =
      await startFormalWorkshopBattle(
        profession,
        'deleted-resource-status-cert',
      );
