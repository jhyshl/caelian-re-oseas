...
describe('release notes', () => {
  it('从当前版本开始按新到旧返回全部历史版本', () => {
    ...
    expect(releases).toEqual(ALPHA_RELEASE_NOTES.filter(note => Number(note.version.split('.').at(-1)) <= 79));
    expect(releaseNotesFor('alpha', '0.2.0-alpha.90')).toEqual(ALPHA_RELEASE_NOTES);
  ...
  it('Beta 只显示自己的版本公告，不混入 Alpha 历史', () => {
    const releases = releaseNotesFor('beta', '1.28.0-beta.1');
