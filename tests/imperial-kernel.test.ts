...
describe('皇权支线完整酒馆事件链',()=>{
  it('接取/取消追踪/继续追踪/放弃/完成精确控制世界书与浮窗，副 API 按正文更新并写回隐藏历史',async()=>{
    ...
    window.tavern_events={GENERATION_ENDED:'ended',CHAT_CHANGED:'changed'};
    ...
    handlers.get('ended')?.();
    await expect.poll(()=>chat[1]!.mes).toContain('CAELIAN_IMPERIAL_STATE:v1');
    expect((await adapter.chatFloors())![1]).toEqual(before);
    expect(apiMessages).toHaveLength(1);expect(apiMessages[0]![0]!.content).toContain('只维护状态栏');
    ...
    await expect.poll(()=>chat[3]!.mes).toContain('CAELIAN_IMPERIAL_STATE:v1');
    ...
    expect(chat[5]!.mes).toContain('已正式继位：测试玩家');
