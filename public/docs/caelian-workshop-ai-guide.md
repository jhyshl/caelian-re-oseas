...
编辑器中新建的灼烧、中毒、流血、腐蚀、诅咒伤害采用 `nativeStatus: true`，`value` 是每层每跳的攻击倍率。攻击力为 100 时，填写 `0.35` 得到每跳 35 点基础伤害；填写 `35` 则是 35 倍攻击力。实际结算仍计算目标防御和护盾，使用施加时的攻击快照，DOT 不暴击。附加 `scaling` 时，公式结果仍是倍率，之后再乘施加时攻击力。
...
该配置每次施加一层灼烧，每层在目标接下来 5 次行动阶段结束时各结算一次，同类工坊 DOT 最多保留 6 层。回合内施加次数不限。

| 字段／输入 | 填写方式 |
| --- | --- |
| `turns` 持续回合 | 正整数；`-1` 为整场战斗。没有固定两回合或 99 回合的限制 |
| `maxStacks` 可叠加上限 | 非负整数；`0` 为不设上限；旧配置省略此字段时默认 3 |
| 普通 DOT 的 `value` | 攻击倍率，`0.35 = 35%`；随卡牌星级系数变化 |
| 攻击提高等比例效果的 `value` | 小数倍率，`0.2 = 20%` |
| 复用效果选择 `unit: "percent"` | 百分数，`35 = 35%`；DOT 会除以 100 后乘攻击力 |
| 暴击率／暴击伤害的默认数值 | 增加的百分点，`10` 表示增加 10 个百分点 |
...
在卡牌定义上设置 `battleOnly: true`。此牌保存在作品定义中，供战斗积木生成和引用；不计入普通卡的8–16种要求，不允许出现在职业卡池、基础构筑或玩家牌组，也不写入永久卡牌库。生成后可照常使用、检索、弃牌和消耗，战斗结束后消失。通过卡牌字段 `afterUse` 选择使用后去向：`"discard"` 为进入弃牌堆、可在本场战斗继续抽取（旧作品未填写时的默认行为）；`"destroy"` 为效果结算后删除本次使用的卡牌实例，不进入任何牌堆。此设置只对 `battleOnly: true` 的牌生效，保存、导出与导入均保留。
