...
`main` 只能为 `knight`、`mage`、`artisan`、`freelance`。`mechanismIds` 可省略；填写后，玩家还需安装相同 ID 的底层机制包，该职业才会在战斗中启用机制。
...
> 严格遵守《凯利安创意工坊 AI 制作手册》。请制作【职业包 / 效果预设扩展 / 底层机制包】。主题是……，核心循环是……，强度目标是……。先在内部检查 ID 唯一、引用完整、起始牌组有效、没有未列出的效果或任意代码。最终只输出一个有效 JSON 对象，不要输出解释或 Markdown 代码围栏。

职业若依赖新机制，建议让 AI 分别输出两个 JSON 文件：先输出机制包，保存后再要求它只输出职业包，并让职业的 `mechanismIds` 引用机制 ID。两份文件依次导入。
