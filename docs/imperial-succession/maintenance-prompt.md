# 动荡的皇权：状态栏维护提示词（Alpha 79）

以下为运行时代码使用的完整规则。副 API 仍返回经过结构校验的 JSON；程序将其排为中文板块，使用 `|` 分隔，追加到 `<caelian-imperial-state><!--…--></caelian-imperial-state>`。完整上一标签直接取自历史原文，不截断，也不以本地缓存覆盖。
...
普通任务和皇权任务的节点、摘要及状态按最近十个酒馆楼层保留。回退撤回对应楼层的任务赠礼、归还已提交物品；任务结算奖励保留领取凭据，不因重 roll 重复发放。超出保留窗口时只能恢复最早保留的状态，会提示核对。已存在的普通楼层没有旧版保存点时无法补造过去进度。
